...
<form method="POST" class="d-grid gap-2 col-6 mx-auto mt-5 p-5">
    ...
    <div class="form-group mb-2 rounded pt-5">
        <label class="form-label text-white" for="pseudo">Pseudo :</label>
    ...
    <div class="form-group mb-2 rounded">
        <label class="form-label text-white" for="sujet">Sujet :</label>
    ...
    <div class="form-check d-flex justify-content-center m-4">
        ...
        <label class="form-check-label text-white mt-3" for="copy">Envoyer une copie de ce message</label>
    </div>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($_POST['pseudo']) && !empty($_POST['email']) && !empty($_POST['sujet']) && !empty($_POST['message'])) {
        ...
        if (isset($_POST['copy'])) {
            ...
            $retour = mail($expediteur, $sujet, $message);
        }
